...
const FROM_ADDRESS = process.env.ZOHO_MAIL_FROM_ADDRESS || "quotes@pressmark.studio";
...
/* Portal sign-in code. Deliberately contains no license key and no link —
   if it reaches the wrong inbox, it is worth nothing on its own. */
export async function sendSignInCode(toAddress, code, expiryMinutes) {
  const accountId = await getMailAccountId(await currentToken());

  const html = `<!doctype html>
<html><body style="margin:0;padding:24px 12px;background:#f6f5f2;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td align="center">
    <table role="presentation" width="600" cellpadding="0" cellspacing="0"
           style="max-width:600px;width:100%;background:#fff;border:1px solid rgba(170,125,72,.3);">
      <tr><td style="padding:36px 40px;font-family:Inter,'Helvetica Neue',Arial,sans-serif;">
        <p style="margin:0 0 22px;font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:${ACCENT};font-weight:700;">
          Pressmark Studio
        </p>
        <p style="margin:0 0 18px;color:${INK};font-size:17px;">Your sign-in code</p>
        <p style="margin:0 0 24px;color:${MUTED};font-size:15px;line-height:1.7;">
          Enter this code to open your license portal.
        </p>
        <p style="margin:0 0 24px;padding:18px;background:#faf8f5;border:1px solid rgba(170,125,72,.3);
                  text-align:center;font-family:ui-monospace,Menlo,Consolas,monospace;
                  font-size:30px;letter-spacing:.22em;color:${INK};">
          ${escapeHtml(code)}
        </p>
        <p style="margin:0;color:${MUTED};font-size:13px;line-height:1.6;">
          It expires in ${Number(expiryMinutes)} minutes. If you didn't ask for it, ignore this email —
  ...
  const response = await fetch(`${MAIL_DOMAIN}/api/accounts/${accountId}/messages`, {
    method: "POST",
    headers: {
      Authorization: `Zoho-oauthtoken ${await currentToken()}`,
      "Content-Type": "application/json",
    },
    body: JSON.stringify({
      fromAddress: FROM_ADDRESS,
      toAddress,
      subject: "Your Pressmark Studio sign-in code",
      content: html,
      mailFormat: "html",
    }),
  });
...
/*
 * A plain notification to the studio's own inbox — the support chat's email
 * fallback uses it. The caller builds the HTML and is responsible for escaping
 * anything a visitor wrote.
 */
export async function sendMail({ toAddress, subject, html }) {
  const token = await currentToken();
  const accountId = await getMailAccountId(token);
  const response = await fetch(`${MAIL_DOMAIN}/api/accounts/${accountId}/messages`, {
    method: "POST",
    headers: { Authorization: `Zoho-oauthtoken ${token}`, "Content-Type": "application/json" },
    body: JSON.stringify({ fromAddress: FROM_ADDRESS, toAddress, subject, content: html, mailFormat: "html" }),
  });
  /* Status only: Zoho's error body can echo addresses. */
  if (!response.ok) throw new Error(`Zoho Mail send failed with status ${response.status}.`);
}

/* The storefront has no Zoho token in hand the way the quote form does, so
   this mints one on demand. */
