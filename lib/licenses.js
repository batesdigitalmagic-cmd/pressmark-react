...
export async function ensureLicense(input) {
  ...
  const existing = await readLicenseByOrder(orderId);
  ...
  const license = {
    ...
    // time — that would silently rewrite history every release.
  ...
  const stored = await writeLicense(license);
  if (email) await indexLicenseEmail(email, stored.key);
  return { created: stored.key === license.key, license: stored };
}

/** Returns null when nothing has been issued for this order yet. */
export async function getLicenseByOrder(orderId) {
  if (usingRemoteServer) {
    try {
  ...
  return readLicenseByOrder(orderId);
}
...
export async function revokeLicenseByOrder(orderId, reason) {
  ...
  await updateLicense(license.key, { revoked: 1, revoked_reason: reason });
  return true;
}
...
export async function activateDevice(key, deviceId, deviceName) {
  ...
  const license = await readLicenseByKey(key);
  ...
  if (existing) {
    ...
    const updated = await updateLicense(key, { activations });
...
export async function deactivateDevice(key, deviceId) {
  ...
  const updated = await updateLicense(key, { activations: remaining });
...
export async function getLicensesByEmail(email) {
  ...
  const keys = await readLicenseKeysByEmail(email);
  const licenses = await Promise.all(keys.map((key) => readLicenseByKey(key)));
