...
export const workerJob = (job) => ({
  ...
  rowCount: job.rowCount,
  /*
   * The brand colours the customer changed, exactly as validated at submission.
   *
   * Usually empty, and empty is meaningful: a swatch that is not named here is
   * left as the template has it. The conversion to CMYK happens on the worker
   * rather than here — the server has no business knowing what an ink is, and
   * the value InDesign is given should be produced next to the code that hands
   * it over.
   */
  brandColors: job.brandColors ?? {},
