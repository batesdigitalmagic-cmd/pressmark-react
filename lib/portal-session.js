...
export async function createSession(email, mode = "live") {
 ...
 * A token minted in one mode is rejected by the other, so a sandbox session
 * can never be replayed against production licences.
 */
export async function readSession(token, mode = "live") {
  ...
  try {
    ...
    if ((decoded.mode || "live") !== mode) return null;
